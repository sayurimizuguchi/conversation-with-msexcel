...
function updateMessage(res, input, response) {
    if (!response.output) {
        response.output = {};
    } else if (response.context.callXls) {
        var jsonXls = xlsx.utils.sheet_to_json(workbook.Sheets[sheet_name_list[0]]);
        var vendas = jsonXls[0].Sales;
        var dataSend = "Suas vendas foram de: " + vendas;
        response.output.text = dataSend;
        return res.json(response);
    } else if (response.context.callXlsClients) {
        var jsonXls = xlsx.utils.sheet_to_json(workbook.Sheets[sheet_name_list[0]]);
        var vendas = jsonXls[0].Sales;
        var dataSend = "Suas vendas foram de: " + vendas;
        response.output.text = dataSend;
